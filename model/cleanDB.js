...
var connection = mysql.createConnection({
	host: "localhost", //이지우 ip주소는 58.121.58.139
	port: "3306",
	user: "tester1",
	password: "qwer1234",
	database: "arduino",
...
function cleanCoLive() {
	// 최근 2분치만 저정하도록 구현
	var d = new Date();
	var now = moment(d).add("-2", "minute").format(fmt1);
	console.log("co Wipe : " + now);
	connection.query("delete from colive WHERE checkTime < '" + now + "'");
}

function cleanH2hoLive() {
	// 최근 2분치만 저정하도록 구현
	var d = new Date();
	var now = moment(d).add("-2", "minute").format(fmt1);
	console.log("h2ho Wipe : " + now);
	connection.query("delete from h2holive WHERE checkTime < '" + now + "'");
}

function cleanRadonLive() {
	// 최근 2분치만 저정하도록 구현
	var d = new Date();
	var now = moment(d).add("-2", "minute").format(fmt1);
	console.log("radon Wipe : " + now);
	connection.query("delete from radonlive WHERE checkTime < '" + now + "'");
}
